...
def test(language, model_name):
    ...
    for num in range(1, 6):
        with open("eval_output/" + language + "_" + model_name + '_gpt2ourall' + str(num) + '.txt', "w+",
                  encoding="utf-8") as f:
            for name, get_data in zip(name_list, get_data_list):
                mrr = []
                ndcg = []
                t = ["model/PA/GPT/gpt2ourall/" + language + "/" + str(nd[name]) + "_GPT_qurey_" + str(i-0) + ".npy" for i in range(num)]
