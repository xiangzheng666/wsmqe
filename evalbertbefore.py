...
def test(language, model_name):
    ...
    for num in range(1, 2):
        with open("eval_output/" + language + "_" + model_name + '_bertbefore' + str(num) + '.txt', "w+",
                  encoding="utf-8") as f:
            with open('result/bertbefore.txt',"w+",encoding="utf-8") as f2:
                for name, get_data in zip(name_list, get_data_list):
                    ...
                    t = ["model/PA/Bert/bertbefore/" + language + "/" + str(nd[name]) + "_bert_qurey_" + str(i-0) + ".npy" for i in range(num)]
