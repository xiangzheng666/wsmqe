...
def test(language, model_name):
    ...
    for num in range(1, 10):
        with open("eval_output/" + language + "_" + model_name + '_bertafter_gptafter' + str(num) + '.txt', "w+",
                  encoding="utf-8") as f:
            with open('result/bertafter_gptafter.txt',"w+",encoding="utf-8") as f2:
                for name, get_data in zip(name_list, get_data_list):
                    print(model_name+"  "+name+"  "+str(num)+"=====================================")
                    mrr = []
                    mrrlist = []
                    ndcg=[]
                    ndcglist = []
                    topk = [[] for i in range(10)]
                    t = ["model/PA/Bert/bertafter/" + language + "/" + str(nd[name]) + "_bert_qurey_" + str(i-0) + ".npy" for i in range(num)]
                    t.extend(["model/PA/GPT/gpt2all/" + language + "/" + str(nd[name]) + "_GPT_qurey_" + str(i-0) + ".npy" for i in range(num)])
                    for code, func, qurey, qurey_o in get_data(qurey_path=t, get_qurey=True):
                        tmp = model.evalGPT([code, func, qurey, qurey_o], b=1)
