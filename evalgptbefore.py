...
def test(language, model_name):
    ...
    for num in range(1, 10):
        with open("eval_output/" + language + "_" + model_name + '_gpt2before' + str(num) + '.txt', "w+",
                  encoding="utf-8") as f:
            with open('result/gpt2before.txt',"w+",encoding="utf-8") as f2:
                for name, get_data in zip(name_list, get_data_list):
                    ...
                    t = ["model/PA/GPT/gpt2before/" + language + "/" + str(nd[name]) + "_GPT_qurey_" + str(i-0) + ".npy" for i in range(num)]
