...
def test(language,model_name):
    ...
    from load_eval_dataset_qurey_desc import get_python
    ...
    with open("eval_output/"+language+"_"+model_name+'_compartor_qurey_desc.txt',"w+",encoding="utf-8") as f:
        for name,get_data in zip(name_list,get_data_list):
            ...
            for code,func,qurey,real_qurey in get_data():
                tmp = model.eval([code, func,qurey])
                real_tmp = model.eval([code, func,real_qurey])
