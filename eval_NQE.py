...
def test(language, model_name):
    ...
    with open("eval_output/" + language + "_" + model_name + '_NQE.txt', "w+",
              encoding="utf-8") as f:
        with open('result/NQE.txt',"w+",encoding="utf-8") as f2:
            for name, get_data in zip(name_list, get_data_list):
                ...
                t="model/PA/NQE/"+language+"/"+str(nd[name])+"_NQE_qurey.npy"
                for code, func, qurey in tqdm(get_data(qurey_path=t)):
